...
begin
  FFastLookupStringList.Clear;  // Do not use the values from SetUp
  FFastLookupStringList.Add('Áccent');
  FFastLookupStringList.Add('áccent');
  FFastLookupStringList.CaseSensitive := False;
  CheckEquals(0, FFastLookupStringList.IndexOf('áccent'), 'IndexOf case-insensitive before locale toggles');
{$IF RTLVersion >= 32.0}
  FFastLookupStringList.UseLocale := False;
  CheckEquals(1, FFastLookupStringList.IndexOf('áccent'), 'IndexOf with UseLocale = False');
  FFastLookupStringList.UseLocale := True;
  CheckEquals(0, FFastLookupStringList.IndexOf('áccent'), 'IndexOf after restoring UseLocale = True');
{$IFEND}
  FFastLookupStringList.CaseSensitive := True;
  CheckEquals(0, FFastLookupStringList.IndexOf('Áccent'), 'IndexOf case-sensitive after toggling back');
end;
